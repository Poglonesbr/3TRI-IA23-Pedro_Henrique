...
const elNumquestao = elQuiz.querySelector(".Numeroquestao")
...
async function main() {
  ...
  function carregarQuestao(numeroDaQuestao) {
    ...
    elNumquestao.innerText = [++numeroDaQuestao] 
    alternativaCorretaAtual = questao.correta
    elPergunta.innerText = questao.pergunta
    elAlternativas.innerHTML = ""
    questao.alternativas.forEach(alt => {elAlternativas.innerHTML += `<button>${alt}  </button>`})
  }
  ...
  function mostrarModalFinish() {
    elModalFinish.classList.add("open")
  }
